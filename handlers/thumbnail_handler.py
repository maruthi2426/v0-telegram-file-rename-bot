...
async def viewthumb_command(client, message: Message):
...
async def delthumb_command(client, message: Message):
...
@app.on_message(filters.photo, group=1)
async def handle_thumbnail_upload(client, message: Message):
...
# Callback handlers
@app.on_callback_query(filters.regex("^upload_thumb$"))
async def upload_thumb_callback(client, query):
    """Handle upload thumbnail button"""
    try:
        user_id = query.from_user.id
        
        await query.edit_message_text("📸 Send me a photo to set as thumbnail")
        thumbnail_states[user_id] = "waiting_thumb"
    except Exception as e:
        logger.error(f"Error in upload_thumb_callback: {e}")

@app.on_callback_query(filters.regex("^change_thumb$"))
async def change_thumb_callback(client, query):
    """Handle change thumbnail button"""
    try:
        user_id = query.from_user.id
        
        await query.edit_message_text("📸 Send me a new photo to update thumbnail")
        thumbnail_states[user_id] = "waiting_thumb"
    except Exception as e:
        logger.error(f"Error in change_thumb_callback: {e}")

@app.on_callback_query(filters.regex("^delete_thumb$"))
async def delete_thumb_callback(client, query):
